...
class NeckDB(nn.Module):
    ...
    def forward(self, x):
        ...
        c2 = self.reduce_conv_c2(c2) # 128 x 160 x 160
        c3 = self.reduce_conv_c3(c3) # 128 x 80 x 80
        c4 = self.reduce_conv_c4(c4) # 128 x 40 x 40
        c5 = self.reduce_conv_c5(c5) # 128 x 20 x 20
        # FPEM
        for i, fpem in enumerate(self.fpems):
            # Up and down sample with Ratio N
...
if __name__ == "__main__":
    ...
    from ..backbone.resnet import BackboneDB
    backbone = BackboneDB()
    y = backbone(x)
