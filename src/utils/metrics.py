...
class PolygonEvaluator(object):
    ...
    def compute_acc(self, poly_pred, poly_gt):
        gt = [{
            'points': [tuple(p) for p in point],
            'ignore': False
        } for point in poly_gt]
        pred = [{
            'points': [tuple(p) for p in point],
            'ignore': False
        } for point in poly_pred]
        result = self.evaluate_image(gt, pred)
        return result
